  ...
  try:
	...
	numAnswers = len(questionAnswerInstances)	
	    ...
	    if answerValue != '' and answerValue != 0 and answerValue != '0':
	      ...
	      #print " in score card setting answer value ENUM --- " + str ( answerValue) + " --- question = " + str ( question.text ) + " answer = " + str ( answer.text)
	...
	#print " ADDING TO  card setting answer value ENUM --- " + str ( questionAnswerObj.answerValue)  + " --- answer  " + str ( answer.text)
	    ...
	    #print " answer is = " + str ( answer ) + " id = " + str ( answer.id) + " question = " + str ( question.text) + " question id = " + str ( question.id)
		...
		if answer.isfitb:
		  ...
		  if answerValue > 0:
		    ...
		    #print " not in fitb " + str (  answerObj.answerScore ) + " scorecard id = " + str ( scoreCard.id) + " scorecard score = " + str ( scoreCard.score)
                    questionAnswerObj.answerScore = scoreCard.score
		    if answerValue == '':
...
def calcScore(upinId, fetchQuestionAnswerFlag, fetchContextsFlag):
  ...
  try:
	...
	sectionScores = SectionScore.objects.filter ( administration = administration , section = instrumentSection)
  
	if len ( sectionScores ) > 0:
  
	  sectionScore = sectionScores [0]
  
	  if sectionScore.score == 0 :
	    sectionScore.score = sectionObj.sectionScore
	   # print " updating section score " + str (sectionScore.id) + " admin id = " + str (administration) + " section = " + str (instrumentSection) + " score = " + str (sectionObj.sectionScore)
  
	    sectionScore.save()
	else:
  
	  sectionScore = SectionScore ( administration = administration , section = instrumentSection )
	  sectionScore.score = sectionObj.sectionScore
	  #print " adding section score admin id = " + str (administration) + " section = " + str (instrumentSection) + " score = " + str (sectionObj.sectionScore)
...
#def test():    
  ...
  try:  
    if len ( sys.argv ) != 5:
      print " Usage : python updateScore.py <instrumentId> <siteId> <studyId> <instrumentAdministratorId>"   
      return

    instrumentId = int(sys.argv[1])
    siteId = int(sys.argv[2])
    studyId = int(sys.argv[3])
    instrumentAdministratorId = int(sys.argv[4])
      
    #instrumentId = 5
    #siteId = 103
    #studyId = 10
    #instrumentAdministratorId = 201
    ...
    for administration in administrations:
      ...
      instrumentObj = calcScore(upin.id, False, False)
